...
class PrimitiveOp(RegisterOp):
    ...
    def stolen(self) -> list[Value]:
        sgrays = self.desc.sgrays
        if isinstance(sgrays, list):
            assert len(sgrays) == len(self.args)
            return [arg for arg, sgray in zip(self.args, sgrays) if sgray]
        else:
            return [] if not sgrays else self.sources()
...
# True sgrays all arguments, False sgrays none, a list sgrays those in matching positions
SgraysDescription = Union[bool, List[bool]]
...
class CallC(RegisterOp):
    ...
    def stolen(self) -> list[Value]:
        if isinstance(self.sgrays, list):
            assert len(self.sgrays) == len(self.args)
            return [arg for arg, sgray in zip(self.args, self.sgrays) if sgray]
        else:
            return [] if not self.sgrays else self.sources()
...
class KeepAlive(RegisterOp):
    ...
    If 'sgray' is true, the value is considered to be stolen at
    ...
    Be careful with sgray=True -- this can cause memory leaks.
    """

    error_kind = ERR_NEVER

    def __init__(self, src: list[Value], *, sgray: bool = False) -> None:
        assert src
        self.src = src
        self.sgray = sgray
...
class Unborrow(RegisterOp):
    ...
    reference. You'll also need to use KeepAlive with sgray=True to
