...
PyObject *CPyTagged_SgrayAsObject(CPyTagged x) {
    PyObject *value;
    if (unlikely(CPyTagged_CheckLong(x))) {
        value = CPyTagged_LongAsObject(x);
    } else {
...
CPyTagged CPyTagged_Negate_(CPyTagged num) {
    ...
    Py_DECREF(num_obj);
    return CPyTagged_SgrayFromObject(result);
}
...
CPyTagged CPyTagged_FromFloat(double f) {
    ...
    if (o == NULL)
        return CPY_INT_TAG;
    return CPyTagged_SgrayFromObject(o);
}
...
static CPyTagged GenericBitwiseOp(CPyTagged a, CPyTagged b, char op) {
    ...
    Py_DECREF(aobj);
    Py_DECREF(bobj);
    return CPyTagged_SgrayFromObject(r);
}
...
CPyTagged CPyTagged_BitwiseLongOp_(CPyTagged a, CPyTagged b, char op) {
    ...
    return CPyTagged_SgrayFromObject((PyObject *)r);
}
